...
(function(j, u, i) {
	j = function(e, h) {
		...
		function l(a, c, b, d) {
			if (!d || a.flatten().length) b.unshift(a), e(a.table().node()).triggerHandler(c + ".dt", b)
		}
		function t(a) {
			var c = a.settings()[0];
			if (c._select.info && c.aanFeatures.i) {
				var b = e('<span class="select-info"/>'),
					d = function(c, d) {
		...
		function r(a, c, b, d, g) {
			...
			"os" === q ? a.ctrlKey || a.metaKey ? c[d](g).select(!f) : a.shiftKey ? "cell" === d ? (d = b._select_lastCell || null, f = function(b, a) {
				...
				if (d.indexOf(b) > d.indexOf(a)) var f = a,
					a = b,
					b = f;
				var g = !1;
				...
				selected: !0
			}).any() && !d ? (f = f(0, g.column), d = a(0, g.row)) : (f = f(d.column, g.column), d = a(d.row, g.row)), d = c.cells(d, f).flatten(), c.cells(g, {
				selected: !0
			}).any() ? c.cells(d).deselect() : c.cells(d).select()) : (a = b._select_lastCell ? b._select_lastCell[d] : null, f = c[d + "s"]({
				search: "applied"
			}).indexes(), a = e.inArray(a, f), b = e.inArray(g, f), !c[d + "s"]({
				selected: !0
			}).any() && -1 === a ? f.splice(e.inArray(g, f) + 1, f.length) : (a > b && (q = b, b = a, a = q), f.splice(b + 1, f.length), f.splice(0, a)), c[d](g, {
				selected: !0
			}).any()) ? (f.splice(e.inArray(g, f), 1), c[d + "s"](f).deselect()) : c[d + "s"](f).select() : (a = c[d + "s"]({
				selected: !0
			}), f && 1 === a.flatten().length ? c[d](g).deselect() : (a.deselect(), c[d](g).select())) : c[d](g).select(!f)
		}
		function p(a, c) {
			return function(b) {
		...
		e.each([{
			...
			h.ext.selector[c.type].push(function(b, a, g) {
				var a = a.selected,
					e, f = [];
				if (a === i) return g;
				for (var k = 0, h = g.length; k < h; k++) e = b[c.prop][g[k]], (!0 === a && !0 === e._select_selected || !1 === a && !e._select_selected) && f.push(g[k]);
				return f
			})
		});
		h.ext.selector.cell.push(function(a, c, b) {
			var c = c.selected,
				d, g = [];
			if (c === i) return b;
			for (var e = 0, f = b.length; e < f; e++) d = a.aoData[b[e].row], (!0 === c && d._selected_cells && !0 === d._selected_cells[b[e].column] || !1 === c && (!d._selected_cells || !d._selected_cells[b[e].column])) && g.push(b[e]);
			return g
		});
		var m = h.Api.register,
			n = h.Api.registerPlural;
		m("select()", function() {});
				...
				if (!c._select_init) {
					...
					c.aoRowCreatedCallback.push({
						fn: function(b, a, d) {
							...
							b = 0;
							for (d = c.aoColumns.length; b < d; b++)(c.aoColumns[b]._select_selected || a._selected_cells && a._selected_cells[b]) && e(a.anCells[b]).addClass("selected")
						},
		...
		e(u).on("init.dt.dtSelect", function(a, c) {
			if ("dt" === a.namespace) {
				var b = c.oInit.select || h.defaults.select,
					d = new h.Api(c),
					g = "row",
					j = "api",
					f = !1,
				...
				c._select = {};
				if (!0 === b) j = "os";
				else if ("string" === typeof b) j = b;
				else if (e.isPlainObject(b) && (b.blurable !== i && (f = b.blurable), b.info !== i && (k = b.info), b.items !== i && (g = b.items), b.style !== i && (j = b.style), b.selector !== i)) l = b.selector;
				d.select.selector(l);
				d.select.items(g);
				d.select.style(j);
				d.select.blurable(f);
